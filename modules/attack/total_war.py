...
x = {}
stage_position = {1: (1155, 225), 2: (1155, 355), 3: (1155, 485), 4: (1155, 165), 5: (1155, 250), 6: (1155, 390), 7: (1155, 530)}


def to_total_war(self):
    pos = {'home_student': (1200, 573), 'home_bus': (926, 450)}
...
def get_prize(self):
    if not color.check_rgb(self, (1241, 628), (251, 64, 16)):
        ...
        return
    pos = {'total_war_menu': (1180, 656), 'total_war_my-info': (925, 170), 'total_war_rank-prize': (240, 310)}
...
def check_ticket(self):
    if image.compare_image(self, 'total_war_no-ticket', threshold=0.9, retry=1):
...
def start_next(self):
    ...
    image.detect(self, 'total_war_menu', pos, rate=1)
    image.compare_image(self, 'total_war_detail-info', cl=(1155, 225), rate=1)
    start_fight(self)
...
def do_exp(self, stage, need_swipe=True):
    ...
    if need_swipe:
        ...
        time.sleep(2)
    image.compare_image(self, 'total_war_detail-info', cl=stage_position[stage], rate=1)
    if not self.tc['help']['enable'] and image.compare_image(self, 'total_war_can-scan', retry=1):
        image.compare_image(self, 'total_war_detail-info', cl=(1121, 166), rate=1, n=True)
        return
    start_fight(self)
...
def start_fight(self):
    image.compare_image(self, 'total_war_force-edit', cl=(1020, 520), rate=1)
    total_select_force(self)
    if force_index == 1:
        pos = {'total_war_force-edit': (1160, 650), 'total_war_need-help': (770, 490)}
        image.detect(self, 'total_war_join-notice', pos, retry=20)
        image.compare_image(self, 'total_war_join-notice', cl=(770, 500), rate=1, n=True, retry=20)
    else:
        image.compare_image(self, 'total_war_force-edit', cl=(1160, 650), rate=1, n=True)
    if force_index == 1:
        stage.wait_loading(self)
        pos = {'momo_talk_menu': (1205, 42), 'momo_talk_skip': (1212, 116), 'momo_talk_confirm-skip': (770, 516)}
        image.detect(self, 'total_war_skip-anm', pos, retry=30, cl=(630, 330))
...
def start_scan(self):
    ...
    if not color.check_rgb(self, (1110, p[1]), (245, 233, 75)):
        return
    image.compare_image(self, 'total_war_detail-info', cl=p, rate=1)
