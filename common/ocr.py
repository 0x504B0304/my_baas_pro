...
def screenshot_get_text(self, area, ocr=None, wait=99999, i=0):
    ...
    if len(out) == 0 and wait > 0:
        return screenshot_get_text(self, area, ocr, wait - 1)
    if len(out) == 0:
        return ''
    return out[i]['text']
...
def screenshot_check_text(self, text, area=(), wait=99999, before_wait=0, need_loading=True, mis_fu=None, mis_argv=None, rate=0.1):
    out = screenshot_cut_get_text(self, area, before_wait, need_loading)
    ex = any(map(lambda d: fuzz.ratio(d.get('text'), text) > 60, out))
    self.logger.info('screenshot_check_text T:%s R:%s', text, ex)
    if ex or wait == 0:
        return ex
...
def screenshot_get_position(self, text, area=(), wait=99999, before_wait=0, need_loading=True):
    ...
    for t in out:
        if fuzz.ratio(t.get('text'), text) > 60:
            self.logger.info('screenshot_check_text T:%s R:%s', text, True)
            return True, t.get('position')
    self.logger.info('screenshot_check_text T:%s R:%s', text, False)
    if wait == 0:
        return False, ()
    time.sleep(self.bc['baas']['base']['ss_rate'])
    if wait < 99999:
